...
function normalizeDateValue(value: unknown): DateModeFilterValue | null {
  ...
  if (Array.isArray(value) && value.length === 2) {
    ...
    const from = fromRaw ? new Date(fromRaw as string | number | Date) : undefined
    const to = toRaw ? new Date(toRaw as string | number | Date) : undefined
...
export function DateModeRowValue({
  ...
  const applyMode = (next: DateFilterMode) => {
    const base: DateModeFilterValue = v ?? ({ mode: next } as DateModeFilterValue)
    ...
    if (next === "from") {
      ...
      nextValue = { mode: "from", from: existingDate, to: undefined }
    } else if (next === "to") {
      const existingDate = base.to ?? base.from
      nextValue = { mode: "to", from: undefined, to: existingDate }
    } else {
      const existingDate = base.from ?? base.to
      if (existingDate) {
        nextValue = {
  ...
  return (
    <>
      <PopoverTrigger isOpen={isOpen} onOpenChange={setIsOpen}>
        ...
        <Popover placement="bottom start" className="overflow-hidden rounded-lg">
          <PopoverDialog className="bg-gray-2 min-w-[140px] p-1">
            <ModeItem active={mode === "from"} onClick={() => applyMode("from")}>
              From
            </ModeItem>
            <ModeItem active={mode === "to"} onClick={() => applyMode("to")}>
              To
            </ModeItem>
            <ModeItem
              active={mode === "range"}
      ...
      <Separator orientation="vertical" className="bg-gray-6 mx-2.5 h-4 w-px" />
