...
// prettier-ignore
const grayScaleNames = ['gray', 'mauve'] as const;
...
export const generateRadixColors = ({
  ...
  const allScalesOklch = Object.entries(allScales)
    .slice(1)
    .map(([scale, colors]) => {
      return {
        name: scale,
        scale: getScaleFromColor(
          new Color(toOklchString(colors[9])).to("oklch"),
          allScales,
          backgroundColor
        ).map(toOklchString),
      }
  ...
  // WDS "warm ink on neutral paper": the neutral gray seed keeps the light
  // SURFACES (steps 1-8) clean, but the design system's INK (steps 9-12) is a
  // warm brown-black. A single-seed Radix ramp can't be neutral-light AND
  // warm-dark, so we overlay the DS ink ramp onto the dark steps in light mode
  // only. Gray is not white-labeled, so these fixed values hold for every tenant.
  // Source: packages/ui/design-system/tokens/{colors,scales}.css (--ds-faint … --ds-ink).
  if (appearance === "light") {
    const ltdsInk = ["#9A8F82", "#8A7F70", "#6B6259", "#2A2520"]
    ltdsInk.forEach((hex, i) => {
      grayScaleColors[8 + i] = new Color(hex).to("oklch")
    })
  }
  ...
  // Limit saturation of the accent TEXT steps so accent-coloured text stays calm
  // and "ink"-like on ANY tenant hue (WDS parity: accent-11 ≈ C0.09, accent-12
  // ≈ C0.06). The stock Radix cap (max chroma of steps 8/9) lets a high-chroma
  // brand — e.g. a periwinkle tenant — render a loud accent label; WDS clamps
  // text chroma to a fixed low value regardless of how saturated the seed is.
  // Only the white-labeled ACCENT text is clamped; the fixed semantic scales keep
  // their tuned text chroma. Lightness is untouched, so WCAG contrast is preserved.
  // Source: packages/ui/design-system/tokens/scales.css (--accent-11 / --accent-12).
  const ACCENT_TEXT_CHROMA_11 = 0.09
  const ACCENT_TEXT_CHROMA_12 = 0.06
  accentScaleColors[10].coords[1] = Math.min(
    ACCENT_TEXT_CHROMA_11,
    accentScaleColors[10].coords[1]
  )
  accentScaleColors[11].coords[1] = Math.min(
    ACCENT_TEXT_CHROMA_12,
...
function getScaleFromColor(
  ...
  const a = colorB?.distance || 0
  const b = colorA?.distance || 0
...
function getAlphaColorP3(
  ...
  const [r, g, b, a] = getAlphaColor(
    ...
    // rounded-md to 255 integers too. Is the browser using 0-255 rather than 0-1 under the hood for P3 too?
    255,
    1000,
    targetAlpha
  )
