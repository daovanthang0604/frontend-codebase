...
// Toast for @workspace/base-ui, built on Base UI's toast primitives
...
// in-viewport list + the `useToast` hook). Base UI has no styling of its own, so
// each toast is a WDS bg-panel card; the manager's `type` drives the status tint.
//
// Usage:
...
type ToastOptions = Omit<ToastManagerAddOptions<Record<string, unknown>>, "title">
...
type AddFn = (options: ToastManagerAddOptions<Record<string, unknown>>) => string
...
function ToastCard({ toast: item }: { toast: ToastItem }) {
  ...
  return (
    <BaseToast.Root
      toast={item}
      className={cn(
        // WDS card: white paper surface, hairline border, soft lift.
        "bg-panel relative flex gap-3 rounded-lg border p-4 shadow-lg",
        ...
        "data-[starting-style]:opacity-0 data-[starting-style]:[transform:translateX(110%)]",
        "data-[ending-style]:opacity-0 data-[ending-style]:[transform:translateX(110%)] data-[ending-style]:duration-200",
