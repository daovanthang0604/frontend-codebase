...
function DialogOverlay({ className }: { className?: string }) {
  return (
    <BaseDialog.Backdrop
      className={cn(
        "bg-gray-a12/80 fixed inset-0 z-50 backdrop-blur-sm",
...
function DialogContent({
  ...
  return (
    <BaseDialog.Portal>
      ...
      <BaseDialog.Popup
        className={cn(
          // WDS: white paper modal (bg-panel) with the bookish 2xl radius,
          // centered. translate centers it, scale drives the zoom — separate css
          // properties in Tailwind v4, so they don't fight.
          "bg-panel fixed top-1/2 left-1/2 z-50 flex max-h-[calc(100dvh-40px)] w-full max-w-[calc(100vw-40px)] -translate-x-1/2 -translate-y-1/2 flex-col gap-4 rounded-2xl p-5 shadow-2xl outline-none md:max-w-lg dark:border",
