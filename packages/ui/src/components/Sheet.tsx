...
const SheetOverlay = ({
  ...
  <AriaModalOverlay
    ...
    className={composeRenderProps(className, (className) =>
      cn(
        // WDS warm-ink scrim (never pure black).
        "bg-gray-a12/60 fixed inset-0 z-50 backdrop-blur-sm duration-300",
        /* Exiting */
...
const SheetContent = ({
  ...
  <SheetOverlay>
    <AriaModal
      className={composeRenderProps(className, (className) =>
        cn(
          // WDS: white paper surface (bg-panel).
          "bg-panel data-[entering]:animate-in data-[exiting]:animate-out fixed z-50 flex flex-col gap-4 shadow-lg transition duration-300 ease-in-out",
          side === "right" &&
